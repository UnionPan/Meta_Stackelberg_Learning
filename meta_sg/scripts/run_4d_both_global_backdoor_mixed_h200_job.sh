...
# 4D Meta-SG run with each task appearing once per outer iteration:
#   clean
#   ipm, lmp, rl
#   bfl, dba, rl_backdoor
#   mixed_backdoor
#
# The defender action is:
...
echo "[job] 4D clean/global/backdoor/mixed: T=$T K=$K H=$H L=$L"
echo "[job] tasks=clean,ipm,lmp,rl,bfl,dba,rl_backdoor,mixed_backdoor"
...
PYTHONPATH=. .venv/bin/python meta_sg/scripts/run_meta_sg_pretraining.py \
  --backend fl_sandbox \
  --attack-domain clean_global_backdoor_mixed \
  --T "$T" \
  --K "$K" \
  --H "$H" \
  --l "$L" \
