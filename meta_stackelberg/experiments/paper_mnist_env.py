...
def make_paper_mnist_env(
    *,
    ...
    partition_rng = RandomSource(seed + 1)
    ...
    return PaperBSMGEnv(
        ...
        sampler=UniformClientSampler(workers),
        benign_trainer=TorchLocalTrainer(
        ...
        population=FixedMaliciousPopulation(malicious_ids),
        model_factory=model_factory,
        codec=codec,
        attacker_dataset=ConcatDataset(malicious_datasets),
        attacker_num_examples={
            client_id: len(client_datasets[client_id]) for client_id in malicious_ids
        },
