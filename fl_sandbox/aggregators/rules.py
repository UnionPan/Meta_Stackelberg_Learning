"""Robust server-side aggregation rules for FL benchmark runs."""
...
def norm_bounded_trimmed_mean_aggregate(
    old_weights: Weights,
    ...
    *,
    ...
    """Paper-style aggregation: norm bounding followed by coordinate trimmed mean."""
    updates = _clip_updates(_stack_updates(old_weights, new_weights), max_norm=norm_bound)
    num_updates = updates.shape[0]
    trim = min(int(num_updates * trim_ratio), max(0, (num_updates - 1) // 2))
    if trim == 0:
        mean_update = np.mean(updates, axis=0)
...
def krum_aggregate(old_weights: Weights, new_weights: List[Weights], num_attackers: int) -> Weights:
    updates = _stack_updates(old_weights, new_weights)
    chosen_update = updates[_krum_candidate_indices(updates, num_attackers=num_attackers)[0]]
    ...
    def aggregate(
        ...
        *,
        ...
        if defense_type == "geometric_median":
            return geometric_median_aggregate(old_weights, new_weights, max_iters=self.geometric_median_iters)
        if defense_type == "trimmed_mean":
            return trimmed_mean_aggregate(old_weights, new_weights, trim_ratio=self.trimmed_mean_ratio)
        if defense_type == "paper_norm_trimmed_mean":
            return norm_bounded_trimmed_mean_aggregate(
                old_weights,
                new_weights,
                norm_bound=self.clipped_median_norm,
...
@dataclass
class PaperActionDefender:
    """Per-round paper defense adapter: alpha norm bound + beta trimmed mean."""
