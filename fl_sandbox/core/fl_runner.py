...
from .metrics import update_norm
from .runtime import (
    RoundRuntimeState,
    RoundSummary,
    RoundTimer,
...
class MinimalFLRunner:
    ...
    def run_round(
            ...
            if self.config.parallel_clients > 1:
                for cid, weights, train_loss, train_acc in self._train_clients_parallel(
                    ...
                    round_state.record_client_training(
                        ...
                        train_acc=train_acc,
                        update_norm=update_norm(old_weights, weights),
                    )
            else:
        ...
        if all_weights:
            self.current_weights = self.defender.aggregate(
                old_weights,
                all_weights,
        ...
        return build_round_summary(
            ...
            defense_name=self.defender.defense_type,
    ...
    def _trusted_reference_update(self, old_weights: List[np.ndarray]) -> Optional[List[np.ndarray]]:
    ...
    def _prepare_poisoned_train_loaders(self) -> Dict[str, object]:
        ...
        loaders: Dict[str, object] = {
            "global_by_attacker": {},
            "sub_triggers_by_attacker": {},
        }
        attacker_union_indices = sorted(
            idx
            for attacker_id in self.attacker_ids
            for idx in self.client_data_idxs[attacker_id]
        )
        if attacker_union_indices:
            ...
            poison_dataset(
                ...
                data_idxs=attacker_union_indices,
        ...
        for attacker_id in self.attacker_ids:
            ...
            poison_dataset(
                local_global_dataset,
                self.config.dataset,
                self.config.base_class,
                self.config.target_class,
                poison_frac=self.config.bfl_poison_frac,
                pattern_type=self.config.pattern_type,
                data_idxs=local_indices,
                poison_all=self.config.bfl_poison_frac >= 1.0,
            )
            ...
            for sub_idx in range(max(1, self.config.dba_num_sub_triggers)):
                ...
                local_sub_trigger_loaders.append(
                    DataLoader(
    ...
    def _initialize_model_pair(self) -> tuple[torch.nn.Module, torch.nn.Module]:
        model = self._build_model().to(self.device)
    ...
    def _load_checkpoint_into_model(self, model: torch.nn.Module, checkpoint_path: str) -> None:
        ...
        if isinstance(checkpoint, (list, tuple)):
            ...
            return
        raise ValueError(
            f"Unsupported checkpoint format at {checkpoint_file}. "
            "Expected a state_dict, a checkpoint containing state_dict/model_state_dict, "
            "a torch.nn.Module, or a list of numpy/tensor weights."
        )
    ...
    @staticmethod
    def _looks_like_state_dict(model: torch.nn.Module, payload: Dict[object, object]) -> bool:
    ...
    def _normalize_state_dict_keys(
        ...
        if set(normalized.keys()) != set(expected_keys):
            raise ValueError(
                "Checkpoint state_dict keys do not match model architecture. "
                f"Expected keys for {model.__class__.__name__}."
            )
        model_state = model.state_dict()
        return {
            key: torch.as_tensor(normalized[key], dtype=model_state[key].dtype)
    ...
    def _split_data(self) -> List[set]:
        if self.config.split_mode.lower() == "iid":
            return self._split_data_iid()
        if self.config.split_mode.lower() == "noniid":
            return self._split_data_noniid()
        if self.config.split_mode.lower() == "paper_q":
    ...
    def _split_data_noniid(self) -> List[set]:
        ...
        for idx, label in enumerate(targets):
            ...
            if rng.random() < q:
                ...
                other_groups = [group_id for group_id in range(num_groups) if group_id != preferred_group]
        ...
        for group_id, indices in grouped_indices.items():
            ...
            for offset, sample_idx in enumerate(indices):
                client_id = clients[offset % len(clients)]
                client_data_idxs[client_id].add(sample_idx)

        return client_data_idxs

    def _split_data_paper_q(self) -> List[set]:
        """Match the reference repo's q-based class grouping more closely."""
        targets = self._dataset_targets()
        classes = sorted({int(label) for label in targets})
        num_groups = len(classes)
        class_to_group = {label: idx for idx, label in enumerate(classes)}
        ...
        for idx, label in enumerate(targets):
            ...
            if rng.random() < keep_label_prob:
                assigned_group = label_group
            else:
                assigned_group = rng.randrange(num_groups)
            grouped_indices[assigned_group].append(idx)
