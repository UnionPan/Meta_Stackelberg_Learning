"""Paper-inspired RL attacker: TD3 policy + gradient distribution learning.

Implements:
1. Learn proxy benign-data distribution from observed server updates.
2. Build a simulated FL environment with that proxy distribution.
3. Train a continuous attacker policy with TD3.
...
class ProxyDatasetBuffer:
    """Replay-style buffer for proxy benign data samples."""
...
class ConvDenoiser(nn.Module):
    """Tiny convolutional denoiser for reconstructed proxy samples."""
...
class GradientDistributionLearner:
    ...
    def _invert_gradient(
        ...
        dummy_x = torch.empty((batch_size, *self.buffer.image_shape), device=self.device).uniform_(
            self.buffer.input_min, self.buffer.input_max,
        )
        ...
        for _ in range(max(1, self.config.inversion_steps)):
            ...
            loss = 1.0 - F.cosine_similarity(
                dummy_grad_vec.unsqueeze(0),
                target_grad[: dummy_grad_vec.numel()].unsqueeze(0),
            ).mean()
...
class Actor(nn.Module):
    def __init__(self, state_dim: int, action_low: np.ndarray, action_high: np.ndarray) -> None:
        ...
        self.net = nn.Sequential(
            ...
            nn.Linear(256, action_low.shape[0]), nn.Tanh(),
        )
...
class TD3Agent:
    """Lightweight TD3 implementation tailored to the attacker simulator."""
...
class SimulatedFLEnv:
    ...
    def __init__(
        ...
        *,
        ...
        self.max_attackers_sampled = max(
            1, min(fl_config.num_attackers, max(1, int(fl_config.num_clients * fl_config.subsample_rate)))
        )
    ...
    def step(self, action: np.ndarray) -> tuple[np.ndarray, float, bool]:
        ...
        reward = (1.0 - bypass_weight) * base_reward + bypass_weight * bypass_reward
    ...
    def _compute_bypass_reward(self, benign_weights: list[Weights], malicious_weights: list[Weights]) -> float:
...
class RLAttackerConfig:
    ...
    def decode_action(self, action: np.ndarray, defense_type: str) -> DecodedAction:
        ...
        if strategy in {"clipmed_ls", "coord_ls"}:
            action_arr = self._normalized_action(action, 3)
        ...
        if strategy == "krum_stealth":
            action_arr = self._normalized_action(action, 3)
            gamma_scale = float(action_arr[0]) * self.krum_gamma_scale + self.krum_gamma_center
            local_steps = int(round(float(action_arr[1]) * self.krum_steps_scale + self.krum_steps_center))
            lambda_stealth = float(action_arr[2]) * self.krum_stealth_scale + self.krum_stealth_center
            return DecodedAction(
                gamma_scale=max(0.1, gamma_scale),
                local_steps=max(1, local_steps),
                lambda_stealth=float(np.clip(lambda_stealth, 0.0, 1.0)),
        ...
        action_arr = self._normalized_action(action_arr, 3)
    ...
    def _normalized_action(self, action: np.ndarray, dim: int) -> np.ndarray:
        action_arr = np.asarray(action, dtype=np.float32)
...
def local_search_update(
    *,
    ...
    benign_tail_grads = torch.autograd.grad(
        benign_loss,
        [param for _, param in _tail_named_parameters(model, state_tail_layers)],
        retain_graph=False,
    )
